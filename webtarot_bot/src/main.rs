...
pub fn main() {
    ...
    let app = App::new("Webtarot Bot")
        ...
        .arg(
            Arg::with_name("json")
                ...
                .long("json")
                .value_name("JSON")
                .help("Deal state json to read")
                .takes_value(true),
        )
        .arg(
            Arg::with_name("joincode")
                .short("j")
                .long("join_code")
                .value_name("JOINCODE")
                .help("Game join code")
    ...
    let json = matches.value_of("json").unwrap_or("");
    let str_websocket = matches
        .value_of("websocket")
        .unwrap_or("ws://127.0.0.1:8001");
    let count = matches
        .value_of("count")
        .and_then(|str_count| str_count.parse::<usize>().ok())
        .unwrap_or(1);

    if let Some(str_socket) = matches.value_of("socket") {
        socket_listener::start(str_socket, str_websocket);
    } else if let Some(json) = matches.value_of("json") {
        explorer::read_json(&json);
    } else {
        match str_command {
