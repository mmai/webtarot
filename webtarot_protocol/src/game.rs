...
use tarotgame::{bid, cards, deal, deal::DealState, pos, trick, Announce};
pub use webgame_protocol::GameEventsListener;
use webgame_protocol::{GameManager, GameState, PlayerInfo, ProtocolErrorKind};

use crate::deal::Deal;
pub use crate::deal::DealSnapshot;
use crate::message::{DebugOperation, TarotVariant};
...
pub struct TarotGameState {
    ...
    turn: Turn,
    deal: Deal,
    deal_ini: Option<DealState>,
    pub deal_history: Vec<(Uuid, PlayCommand)>,
    first: pos::PlayerPos,
    scores: Vec<Vec<f32>>,
}
//
...
impl<'a, Listener: GameEventsListener<(PlayEvent, TarotGameState)> + PartialEq>
    TarotGameManager<'a, Listener>
{
    ...
    pub fn set_play(&mut self, pid: Uuid, card: cards::Card) -> Result<(), ProtocolError> {
        ...
        let state = self.state.deal.deal_state_mut().ok_or(ProtocolError::new(
        ...
        if let Some(deal_state) = self.state.deal.deal_state() {
            if Some(card) == deal_state.king()
                && self.state.get_player_role(pid) != Some(PlayerRole::Taker)
            {
...
impl<'a, Listener: GameEventsListener<(PlayEvent, TarotGameState)> + PartialEq>
    GameManager<'a, Listener> for TarotGameManager<'a, Listener>
{
    ...
    fn emit(&mut self, event: Self::Event) {
        self.listeners
            .iter_mut()
            .for_each(|listener| listener.notify(&event));
    }
}
...
impl GameState for TarotGameState {
    ...
    fn player_by_pos(&self, position: pos::PlayerPos) -> Option<&GamePlayerState> {
        self.players
            .iter()
            .find(|(_uuid, player)| player.pos == position)
            .map(|p| p.1)
    }
    ...
    fn make_snapshot(&self, player_id: Uuid) -> GameStateSnapshot {
        ...
        let deal = match self.deal.deal_state() {
            Some(state) => {
                ...
                if let deal::DealResult::GameOver {
                    taker_diff: diff,
                    scores: lscores,
                    ..
                } = state.get_deal_result()
                {
                    scores = lscores;
                    taker_diff = diff;
                    dog = state.dog();
                ...
                if matches!(self.turn, Turn::Playing(_x)) {
                    ...
                    let to_show: Vec<cards::Card> = state
                        .dog()
                        .list()
                        .into_iter()
                        .filter(|c| {
                            c.suit() == cards::Suit::Trump || c.rank() == cards::Rank::RankK
                        })
                        .collect();
                    for c in to_show {
                        dog.add(c);
    ...
    fn set_player_ready(&mut self, player_id: Uuid) -> bool {
        if let Some(player_state) = self.players.get_mut(&player_id) {
            ...
            if count == self.nb_players {
                self.turn =
                    Turn::Bidding((bid::AuctionState::Bidding, pos::PlayerPos::from_n(0, count)));
                return true;
            }
        }
        false
    ...
    fn manage_operation(&mut self, operation: Self::Operation) -> Option<String> {
        match operation {
            Self::Operation::SetSeed(seed) => {
                let (hands, dog) = tarotgame::deal_seeded_hands(seed, self.nb_players as usize);
                self.deal
                    .deal_auction_mut()
                    .map(|auction| auction.set_hands(hands, dog));
                None
            }
    ...
    fn debug_bychat(&mut self, operation: &str) -> Option<String> {
        // println!("Debug state : {}", serde_json::to_string(self).unwrap());
        if operation == "debugbot" {
            serde_json::to_string(self).ok()
        } else {
...
impl TarotGameState {
    ...
    pub fn players_ready(&self) -> bool {
        !(self
            .players
            .iter()
            .find(|(_, player)| player.ready == false)
            != None)
    }
    ...
    pub fn set_pass(&mut self, pid: Uuid) -> Result<(), ProtocolError> {
        ...
        let auction = self.deal.deal_auction_mut().ok_or(ProtocolError::new(
            ProtocolErrorKind::InternalError,
            "unknown auction",
        ))?;
    ...
    pub fn set_announce(&mut self, pid: Uuid, announce: Announce) -> Result<(), ProtocolError> {
        let pos = self.players.get(&pid).map(|p| p.pos).unwrap();
        let state = self.deal.deal_state_mut().ok_or(ProtocolError::new(
            ProtocolErrorKind::InternalError,
            "Unknown deal state",
        ))?;
    ...
    fn complete_auction(&mut self) -> Result<(), ProtocolError> {
        ...
        self.deal = Deal::Playing(deal_state.clone());
        // Reset play history
        self.deal_ini = Some(deal_state);
        self.deal_history = vec![];
...
impl GameStateSnapshot {
    ...
    pub fn current_player_name(&self) -> String {
        let found_name = self.get_playing_pos().map(|pos| self.pos_player_name(pos));
...
mod tests {
    use super::*;
    use tarotgame::{bid, cards, deal_hands, deal_seeded_hands};
    ...
    fn test_all_pass() {
        ...
        let pos0 = game.add_player(PlayerInfo {
            id: Uuid::new_v4(),
            nickname: String::from("player0"),
        });
        let pos1 = game.add_player(PlayerInfo {
            id: Uuid::new_v4(),
            nickname: String::from("player1"),
        });
        let pos2 = game.add_player(PlayerInfo {
            id: Uuid::new_v4(),
            nickname: String::from("player2"),
        });
        let pos3 = game.add_player(PlayerInfo {
            id: Uuid::new_v4(),
            nickname: String::from("player3"),
        });
        let pos4 = game.add_player(PlayerInfo {
            id: Uuid::new_v4(),
            nickname: String::from("player4"),
        });
        ...
        assert_eq!(
            game.get_turn(),
            Turn::Bidding((bid::AuctionState::Bidding, pos0))
        );
        ...
        assert_eq!(
            game.get_turn(),
            Turn::Bidding((bid::AuctionState::Bidding, pos1))
        );
    ...
    fn test_game() {
        ...
        let seed = [
            3, 32, 3, 32, 54, 1, 84, 3, 32, 54, 1, 84, 3, 32, 65, 1, 84, 3, 32, 64, 1, 44, 3, 32,
            54, 1, 84, 3, 32, 65, 1, 44,
        ];
        ...
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank2),
            )
            .unwrap();
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::RankJ),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank10),
            )
            .unwrap();
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank9),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(id1, cards::Card::new(cards::Suit::Club, cards::Rank::Rank4))
            .unwrap();
        game_manager
            .set_play(id2, cards::Card::new(cards::Suit::Club, cards::Rank::Rank3))
            .unwrap();
        game_manager
            .set_play(id3, cards::Card::new(cards::Suit::Club, cards::Rank::Rank1))
            .unwrap();
        game_manager
            .set_play(id4, cards::Card::new(cards::Suit::Club, cards::Rank::RankC))
            .unwrap();
        game_manager
            .set_play(id0, cards::Card::new(cards::Suit::Club, cards::Rank::Rank5))
            .unwrap();
        // game.set_player_ready(id0);
        // game.set_player_ready(id1);
        // game.set_player_ready(id2);
        ...
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Spade, cards::Rank::Rank6),
            )
            .unwrap();
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Spade, cards::Rank::RankC),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Spade, cards::Rank::Rank5),
            )
            .unwrap();
        // game.set_player_ready(id0);
        // game.set_player_ready(id1);
        ...
        game_manager
            .set_play(id0, cards::Card::new(cards::Suit::Club, cards::Rank::Rank8))
            .unwrap();
        game_manager
            .set_play(
                id1,
            ...
            .unwrap();
        game_manager
            .set_play(id2, cards::Card::new(cards::Suit::Club, cards::Rank::RankJ))
            .unwrap();
        game_manager
            .set_play(id3, cards::Card::new(cards::Suit::Club, cards::Rank::Rank2))
            .unwrap();
        game_manager
            .set_play(id4, cards::Card::new(cards::Suit::Club, cards::Rank::Rank7))
            .unwrap();
        // game.set_player_ready(id0);
        // game.set_player_ready(id1);
        // game.set_player_ready(id2);
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank6),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank10),
            )
            .unwrap();
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank1),
            )
            .unwrap();
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank4),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank14),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank6),
            )
            .unwrap();
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank21),
            )
            .unwrap();
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank22),
            )
            .unwrap();
        // game.set_player_ready(id0);
        // game.set_player_ready(id1);
        ...
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank3),
            )
            .unwrap();
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank5),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank15),
            )
            .unwrap();
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank4),
            )
            .unwrap();
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank13),
            )
            .unwrap();
        ...
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank19),
            )
            .unwrap();
        game_manager
            .set_play(id3, cards::Card::new(cards::Suit::Club, cards::Rank::RankQ))
            .unwrap();
        game_manager
            .set_play(
                id4,
        ...
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank12),
            )
            .unwrap();
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank20),
            )
            .unwrap();
        // assert_eq!(game_manager.get_game().get_turn(), Turn::Intertrick);
        // game.set_player_ready(id0);
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank5),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank18),
            )
            .unwrap();
        game_manager
            .set_play(id3, cards::Card::new(cards::Suit::Club, cards::Rank::Rank9))
            .unwrap();
        game_manager
            .set_play(
                id4,
        ...
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::Rank3),
            )
            .unwrap();
        // assert_eq!(game_manager.get_game().get_turn(), Turn::Intertrick);
        // game.set_player_ready(id0);
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank17),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Heart, cards::Rank::Rank1),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Heart, cards::Rank::Rank3),
            )
            .unwrap();
        game_manager
            .set_play(
                id0,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank11),
            )
            .unwrap();
        // assert_eq!(game_manager.get_game().get_turn(), Turn::Intertrick);
        // game.set_player_ready(id0);
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank16),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Heart, cards::Rank::Rank2),
            )
            .unwrap();
        game_manager
            .set_play(id3, cards::Card::new(cards::Suit::Club, cards::Rank::Rank6))
            .unwrap();
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Heart, cards::Rank::Rank5),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank9),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Spade, cards::Rank::Rank1),
            )
            .unwrap();
        game_manager
            .set_play(id4, cards::Card::new(cards::Suit::Club, cards::Rank::RankK))
            .unwrap();
        game_manager
            .set_play(
                id0,
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Trump, cards::Rank::Rank7),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Spade, cards::Rank::Rank4),
            )
            .unwrap();
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Spade, cards::Rank::Rank3),
            )
            .unwrap();
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::RankC),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        game_manager
            .set_play(
                id1,
                cards::Card::new(cards::Suit::Heart, cards::Rank::Rank6),
            )
            .unwrap();
        game_manager
            .set_play(
                id2,
                cards::Card::new(cards::Suit::Spade, cards::Rank::RankK),
            )
            .unwrap();
        game_manager
            .set_play(
                id3,
                cards::Card::new(cards::Suit::Spade, cards::Rank::RankJ),
            )
            .unwrap();
        game_manager
            .set_play(
                id4,
                cards::Card::new(cards::Suit::Diamond, cards::Rank::RankQ),
            )
            .unwrap();
        game_manager
            .set_play(
        ...
        // game.set_player_ready(id4);
        assert_eq!(
            game_manager.get_game().get_turn(),
            Turn::Bidding((bid::AuctionState::Bidding, pos1))
        );
    ...
    fn test_generic_game() {
        ...
        for v in 0..variant {
            let pos = game.add_player(PlayerInfo {
                id: Uuid::new_v4(),
                nickname: String::from(format!("player{:?}", v)),
            });
        ...
        let seed = [
            7, 32, 3, 32, 54, 1, 84, 3, 32, 54, 1, 84, 3, 32, 65, 1, 84, 3, 32, 64, 1, 44, 3, 32,
            54, 1, 84, 3, 32, 65, 1, 44,
        ];
        ...
        for v in 1..variant {
            ...
            game.set_pass(game.player_by_pos(pos).unwrap().player.id)
                .unwrap();
        }
        if variant == 5 {
            assert_eq!(game.get_turn(), Turn::CallingKing);
        ...
        for _id in 0..deal_size {
            ...
            for _v in 0..variant {
                ...
                for card in hand.list() {
                    // Try to play a card until one is correct
